...
如果完成的这一回合新增了文件，桥接器会在文字结果之后自动发送这些文件：常见图片格式显示为微信图片，其他格式显示为文件附件。只发送本轮新增的文件，不会把修改过的已有文件全部发一遍。
...
- 命令执行和文件变更仍可在微信中逐项审批。
- 自动附件只允许来自已配置项目目录的普通文件；`.env`、密钥、凭据、符号链接、项目外文件和超过限制的文件会被跳过并提示原因。
- 默认每个任务最多发送 5 个附件，每个附件最大 20 MB；可在配置中关闭或调整限制。
- 插件不会绕过系统操作确认或微信首次授权。

## Desktop/CLI 任务通知

桥接器默认每 15 秒只读检查已配置项目下的 Desktop 和 CLI 任务。首次启动只建立历史基线，不会把旧结果全部发到微信；之后新出现的 `completed`、`failed` 和 `interrupted` 回合会自动通知。完成回合中识别到的新增文件也会随通知一起发送。
