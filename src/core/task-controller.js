...
const HELP = `微信 Codex 命令
/new [项目]  新建任务
/tasks       查看最近任务
/active      查看微信端运行任务
/recent      发现 Desktop/CLI 任务
/unread      查看未读完成消息
/read M1     标记消息已读（支持 all）
/use 编号    切换任务（支持 C1）
/status      查看当前状态
/cancel      停止当前运行
/approve A1  允许待审批操作
...
直接发送文字，会继续当前任务；当前任务仍在运行时，文字会作为补充要求加入。`;
...
export class TaskController {
  ...
  async #route(userId, text, contextToken) {
    ...
    switch (command.toLowerCase()) {
      case "/help": case "/帮助": return this.#send(userId, HELP, contextToken);
