...
1. Resolve the current task's real project root automatically. Ask only when there is no project context or two plausible roots exist. Never choose this plugin's cache or marketplace checkout.
2. Tell the user, in plain language, that Codex will configure the current project and install a local auto-start service. Do not expose filesystem details unless they ask. Obtain any approval required by the host immediately before setup.
...
Running `install` again safely updates the stable runtime and adds the selected project without erasing existing projects or credentials. Never instruct the user to edit `config.json`; rerun setup for each project they want to add.
